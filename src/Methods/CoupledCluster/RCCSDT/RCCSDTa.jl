...
include("RCCSDTHelper.jl")

function RCCSDT(alg::RCCSDTa)
    aoints = IntegralHelper{Float64}()
    RCCSDT(aoints, alg)
end

function RCCSDT(mol::Molecule, alg::RCCSDTa)
    aoints = IntegralHelper{Float64}(molecule=mol)
    RCCSDT(aoints, alg)
end

function RCCSDT(aoints::IntegralHelper, alg::RCCSDTa)
    rhf = RHF(aoints)
    ...
    RCCSDT(moints, aoints, alg)
end

function RCCSDT(moints::IntegralHelper{T,<:AbstractERI,<:AbstractRestrictedOrbitals}, aoints::IntegralHelper{T,<:AbstractERI,AtomicOrbitals}, alg::RCCSDTa) where T<:AbstractFloat
...
function RCCSDT(moints::IntegralHelper{T,E1,<:AbstractRestrictedOrbitals}, aoints::IntegralHelper{T,E2,AtomicOrbitals}, alg::RCCSDTa) where {T<:AbstractFloat,E1<:AbstractDFERI,E2<:AbstractDFERI}
    Fermi.Integrals.compute!(moints, aoints, "F")
...
function RCCSDT(moints::IntegralHelper{T,E,O}, alg::RCCSDTa) where {T<:AbstractFloat,E<:AbstractERI,O<:AbstractRestrictedOrbitals}
    ...
    T3guess = zeros((o,o,o,v,v,v))#AbstractArray{T, 6}(undef, o*v*o*v*o*v)
    ...
    RCCSDT(moints, T1guess, T2guess, T3guess, alg)
end

function RCCSDT(moints::IntegralHelper{T,E,O}, newT1::AbstractArray{T,2}, newT2::AbstractArray{T,4}, 
    newT3::AbstractArray{T, 6}, alg::RCCSDTa) where {T<:AbstractFloat, E<:AbstractERI, O<:AbstractRestrictedOrbitals}
    ...
    if (cc_e_conv < eps(T) || cc_max_rms < eps(T)) && !(precision_override)
        ...
        output("\t             CCSDT is unlikely to converge to your standards.")
    ...
    if do_diis
        ...
        DM_T3 = Fermi.DIIS.DIISManager{T, diis_prec}(size=ndiis)
    end
    ...
    while (abs(dE) > cc_e_conv || rms > cc_max_rms)
        ...
        t = @elapsed begin
            ...
            #debug
            update_amp!(newT1, newT2, newT3, T1, T2, T3, moints, alg)

            # Compute residues 
            r1 = sqrt(sum((newT1 .- T1).^2)/length(T1))
            r2 = sqrt(sum((newT2 .- T2).^2)/length(T2))
            r3 = sqrt(sum((newT3 .- T3).^2)/length(T3))
            ...
            if do_diis
                ...
                if ite ≥ diis_start && ite % diis_relax == 0
                    ...
                    #newT3 = Fermi.DIIS.extrapolate(DM_T3;add_res=true)
    ...
    output("\nMain CCSDT iterations done in {:5.5f} s", main_time)
    ...
    output(" @Final CCSDT Correlation Energy:     {:15.10f}", Ecc)
    output(" @Final CCSDT Energy:                 {:15.10f}", Ecc+Eref)
    output(repeat("-",80))

    if Options.get("return_ints")
        return RCCSDT(Eguess, Ecc, Ecc+Eref, newT1, newT2, newT3, dE, rms), moints
    else
        return RCCSDT(Eguess, Ecc, Ecc+Eref, newT1, newT2, newT3, dE, rms)
    end
end